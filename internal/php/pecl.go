...
func (p *PeclRecipe) Build(ctx context.Context, ext Extension, ec ExtensionContext, run runner.Runner) error {
	...
	return buildPecl(ctx, ext.Name, ext.Version, url, ec, p.configureOptions(ec), run)
}
...
func (a *AmqpPeclRecipe) Build(ctx context.Context, ext Extension, ec ExtensionContext, run runner.Runner) error {
	...
	return buildPecl(ctx, ext.Name, ext.Version, url, ec, opts, run)
}
...
func (m *MaxMindRecipe) Build(ctx context.Context, ext Extension, ec ExtensionContext, run runner.Runner) error {
	...
	return buildPeclInSubdir(ctx, ext.Name, ext.Version, url, fmt.Sprintf("maxminddb-%s/ext", ext.Version), ec, opts, run)
}
...
func (t *TidewaysXhprofRecipe) Build(ctx context.Context, ext Extension, ec ExtensionContext, run runner.Runner) error {
	...
	return buildPeclInSubdir(ctx, ext.Name, ext.Version, url, fmt.Sprintf("php-xhprof-extension-%s", ext.Version), ec, opts, run)
}
...
func buildPecl(ctx context.Context, name, version, url string, ec ExtensionContext, opts []string, run runner.Runner) error {
	return buildPeclInSubdir(ctx, name, version, url, fmt.Sprintf("%s-%s", name, version), ec, opts, run)
}

// buildPeclInSubdir is like buildPecl but uses a custom subdirectory inside the extracted archive.
func buildPeclInSubdir(ctx context.Context, name, version, url, subdir string, ec ExtensionContext, opts []string, run runner.Runner) error {
